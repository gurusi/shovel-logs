...
parse_args() {
  ...
  local long_args="config:,debug,destination:,no-check-deps,no-abort,help,mv-args:,print-config,print-valid-config,syslog,simulate,verbose"
...
These options are MANDATORY:
  -t, --move-to : The destination directory to move logs to, Current: "$CONF_MOVE_TO"
      --mv-args : Arguments given to mv, current: "$CONF_MOVE_ARGS"
...
check_CONF_MOVE_TO() {
  ...
  [ -L "$CONF_MOVE_TO" ] && {
    log_warning "Destination dir \"$CONF_MOVE_TO\" is a symlink, resolving."
    CONF_MOVE_TO=$(readlink -f "$CONF_MOVE_TO") || {
      log_error "Could not resolve \"$CONF_MOVE_TO\"."
      return 1
    }
  }
  ...
  [ -d "$CONF_MOVE_TO" ] || {
    log_error "Destination dir \"$CONF_MOVE_TO\" is not a directory."
    return 1
  }
  [ -w "$CONF_MOVE_TO" ] || {
    log_error "Destination dir \"$CONF_MOVE_TO\" is writable."
    return 1
  }
  return 0
}
...
check_CONF_MOVE_TO; errors=$(( $errors + $? ))
...
# Move the logfiles; do the stuff if we are not in simulate mode
run="mv $CONF_MOVE_ARGS $CONF_LOG_FILES $CONF_MOVE_TO"
if do_simulate; then
  log "Would run: \"$run\"."
else
  log_debug "Running: '$run'"
  eval $run || {
    die_or_warn "Failed moving."
  }
fi
