...
parse_args() {
  ...
  local long_args="config:,debug,no-check-deps,no-abort,help,print-config,print-valid-config,syslog,simulate,verbose"
...
run="nice -n19 rm $CONF_RM_ARGS $CONF_LOG_FILES"
if do_simulate; then
  log "Would run: \"$run\"."
else
  log_debug "Running: '$run'"
  eval $run || {
    die_or_warn "Failed deleting."
  }
fi
