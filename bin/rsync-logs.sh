...
parse_args() {
  ...
  local long_args="config:,debug,destination:,no-check-deps,no-abort,help,print-config,print-valid-config,rsync-args:,syslog,simulate,verbose"
  ...
  while true; do
    ...
    # Destination
    elif [ "$a" = "-t" -o "$a" = "--rsync-to" ]; then
      shift; CONF_RSYNC_TO="$1"

    # Rsync arguments
    elif [ "$a" = "--rsync-args" ]; then
      shift; CONF_RSYNC_ARGS="$1"
...
These options are MANDATORY:
  -t, --rsync-to   : The destination directory to rsync logs to, Current: "$CONF_RSYNC_TO"
      --rsync-args : Arguments given to rsync, current: "$CONF_RSYNC_ARGS"
...
check_CONF_RSYNC_TO() {
  [ -z "$CONF_RSYNC_TO" ] && {
    log_error "No destination given."
    return 1
  }
  return 0
}
...
check_CONF_RSYNC_TO; errors=$(( $errors + $? ))
...
run="nice -n19 rsync $CONF_RSYNC_ARGS $CONF_LOG_FILES $CONF_RSYNC_TO"
if do_simulate; then
  log "Would run: \"$run\"."
else
  log_debug "Running: '$run'"
  eval $run || {
    die_or_warn "Failed rsyncing."
  }
fi
