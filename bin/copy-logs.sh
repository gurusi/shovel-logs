...
parse_args() {
  ...
  local long_args="config:,copy-args:,debug,destination:,no-check-deps,no-abort,help,print-config,print-valid-config,syslog,simulate,verbose"
  ...
  while true; do
    ...
    # Destination
    elif [ "$a" = "-t" -o "$a" = "--copy-to" ]; then
      shift; CONF_COPY_TO="$1"

    # Destination
    elif [ "$a" = "--copy-args" ]; then
      shift; CONF_COPY_ARGS="$1"
...
These options are MANDATORY:
  -t, --copy-to   : The destination directory to copy logs to, Current: "$CONF_COPY_TO"
      --copy-args : Arguments given to cp, current: "$CONF_COPY_ARGS"
...
check_CONF_COPY_TO() {
  ...
  # check if the destination is a symlink and resolve it to regular dir
  [ -L "$CONF_COPY_TO" ] && {
    log_warning "Destination dir \"$CONF_COPY_TO\" is a symlink, resolving."
    CONF_COPY_TO=$(readlink -f "$CONF_COPY_TO") || {
      log_error "Could not resolve \"$CONF_COPY_TO\"."
      return 1
    }
  }

  # check if the destination is a regular dir
  [ -d "$CONF_COPY_TO" ] || {
    log_error "Destination dir \"$CONF_COPY_TO\" is not a directory."
    return 1
  }
  [ -w "$CONF_COPY_TO" ] || {
    log_error "Destination dir \"$CONF_COPY_TO\" is writable."
    return 1
  }
  return 0
}
...
# Init#{{{
#
# Check the regular stuff
check_CONF_COPY_TO; errors=$(( $errors + $? ))
...
# Copy the logfiles; do the stuff if we are not in simulate mode
run="nice -n19 cp $CONF_COPY_ARGS $CONF_LOG_FILES $CONF_COPY_TO"
if do_simulate; then
  log "Would run: \"$run\"."
else
  log_debug "Running: '$run'"
  eval $run || {
    die_or_warn "Failed copying."
  }
fi
